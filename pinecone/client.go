...
// [CreateIndexRequest] holds the parameters for creating an index with the native 2026-07
// schema-first surface via [Client.CreateIndex].
//
// Fields:
...
//     characters or '-'. If empty, a name is generated by the server; callers that need retry-safe
//     behavior should provide an explicit name.
//   - Schema: (Required) The [IndexSchema] defining the index's typed fields. Fields accepted at
//     creation time are [DenseVectorField], [SparseVectorField], and [StringField] with a
//     FullTextSearch configuration; metadata fields are indexed automatically at upsert and are not
//     declared. A schema of exactly the reserved fields "_values" (dense) and/or "_sparse_values"
//     (sparse) creates a classic vector index served by the vector operations; any other schema
//     creates a document index served by the documents API ([IndexConnection.UpsertDocuments] etc.).
//   - Deployment: (Optional) The [IndexDeployment] selecting infrastructure. Defaults to managed
...
// [Client.CreateIndex] creates a new [Index] from an explicit [IndexSchema] and [IndexDeployment],
// the native form of index creation under the 2026-07 API. Use this to create document indexes
// (full-text search, hybrid dense+sparse with named fields); the legacy
// [Client.CreateServerlessIndex] and [Client.CreatePodIndex] remain available for classic
// dimension/metric vector indexes.
//
// Parameters:
//   - ctx: A context.Context object controls the request's lifetime, allowing for the request
//     to be canceled or to timeout according to the context's deadline.
...
//				    "embedding":    {DenseVector: &pinecone.DenseVectorField{Dimension: 1536, Metric: pinecone.Dotproduct}},
//				    "sparse_terms": {SparseVector: &pinecone.SparseVectorField{}},
//			    },
//		    },
//		    Deployment: &pinecone.IndexDeployment{
//			    Managed: &pinecone.ManagedDeployment{Cloud: pinecone.Aws, Region: "us-east-1"},
//		    },
//	    })
...
func (c *Client) CreateServerlessIndex(ctx context.Context, in *CreateServerlessIndexRequest) (*Index, error) {
	...
	if in.Schema != nil {
		return nil, fmt.Errorf("Schema (metadata schema) is not supported by the 2026-07 API: metadata fields are indexed automatically at upsert, so there is nothing to declare at create time")
	}
...
func (c *Client) ConfigureIndex(ctx context.Context, name string, in ConfigureIndexParams) (*Index, error) {
	if in.PodType == "" && in.Replicas == 0 && in.DeletionProtection == "" && in.Tags == nil && in.ReadCapacity == nil && in.Embed == nil {
		return nil, fmt.Errorf("must specify PodType, Replicas, DeletionProtection, ReadCapacity, or Tags when configuring an index")
	}
	if in.Embed != nil {
		return nil, fmt.Errorf("Embed is not supported by the 2026-07 API: the convert-to-integrated flow was removed and the server rejects unknown PATCH fields; embedding configuration is set at creation time via CreateIndexForModel")
	}
...
// [Document] is a map representing a document. It is used both for the documents API on an
...
func toIndex(idx *db_control.IndexModel) (*Index, error) {
	...
	deployment, err := toIndexDeployment(idx.Deployment)
	if err != nil {
		return nil, err
	}
...
func toBackup(backup *db_control.BackupModel) *Backup {
	...
	if backup.CreatedAt != nil {
		formatted := backup.CreatedAt.Format(time.RFC3339)
		createdAt = &formatted
	}
